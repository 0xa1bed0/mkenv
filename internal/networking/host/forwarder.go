...
func (f *Forwarder) Start(rt *runtime.Runtime) error {
	...
	f.srv = server
	logs.Infof("[mkenv host] forwarder listening on %s -> container:%d (via proxy at %d)", addr, f.TargetPort, f.ContainerProxyPort)
	return nil
}
...
func (f *Forwarder) handleConn(clientConn net.Conn) {
	...
	if err != nil {
		logs.Infof("[mkenv host] forwarder: DialProxy failed for host port %d: %v", f.TargetPort, err)
		return
	}
	...
	if err := protocol.WriteProxyHeader(backendConn, f.TargetPort); err != nil {
		logs.Infof("[mkenv host] forwarder: header write failed on %d: %v", f.TargetPort, err)
		return
	}
...
func (r *ForwarderRegistry) Remove(targetPort int) {
	...
	if f, ok := r.forwarders[targetPort]; ok {
		logs.Infof("Stop forwarding to %d", targetPort)
		f.Stop()
...
func (r *ForwarderRegistry) StopAll() {
	...
	for port, f := range r.forwarders {
		logs.Infof("Stop forwarding to %d", port)
		f.Stop()
